...
def __execute(command):
    subprocess.call(shlex.split(command), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=dict(os.environ, QT_QPA_PLATFORM="offscreen"))
...
def __icp_command(reference_pcl_path, pcl_to_modify_path, output_path):
    return """CloudCompare \
            ...
            -O {reference} \
            -ICP \
            -SAVE_CLOUDS FILE '{output_path} temp.laz'""".format(
